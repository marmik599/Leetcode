...
bool iscbt(struct Node* root,int i,int countnode)
{
    if(root==NULL)
        return true;
    ...
    if(i>=countnode)
        ...
        return iscbt(root->left,2*i+1,countnode) && iscbt(root->right,2*i+2,countnode);
}
...
{
    int i=0;
    int countnode=totalcount(root);
    
    if(iscbt(root,0,countnode) && maxorder(root))
        return true;
